...
ROFI_TEMPLATE="$CONFIG_DIR/rofi/config-template.rasi"
ROFI_UPDATE="$CONFIG_DIR/rofi/update-rofi.sh"
...
# ===============================
print_info "Selecting wallpaper via Rofi..."
WALL_NAME=$(ls "$WALLPAPERS_DIR" | rofi -dmenu -p "Select Wallpaper:" -theme "$ROFI_TEMPLATE")
...
# Step 5: Update Rofi
# ===============================
if [[ -f "$ROFI_UPDATE" ]]; then
    print_info "Updating Rofi theme..."
    run_as_user "$ROFI_UPDATE"
    print_success "Rofi theme updated."
else
    print_warning "Rofi update script not found at $ROFI_UPDATE."
fi
...
COLOR_BG=$(awk -F: '/background/ {gsub(/[ ;]/,"",$2); print $2}' "$WAL_CSS" | head -1)
COLOR_FG=$(awk -F: '/foreground/ {gsub(/[ ;]/,"",$2); print $2}' "$WAL_CSS" | head -1)
