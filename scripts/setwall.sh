...
# 1. Fire the circular grow transition
awww img "$WALL" \
...
# 2. Pause the script so bash waits for the 0.7s animation to fully finish playing
sleep 0.7

# 3. Now run wal safely without cutting off the transition
wal -i "$WALL" --backend haiku

# 4. Symlink current wallpaper
ln -sf "$WALL" ~/.cache/current-wallpaper

# 5. Folder icon recolor
[[ -f "$HOME/.config/scripts/recolor_folders.sh" ]] && \
    bash "$HOME/.config/scripts/recolor_folders.sh" &

# 6. Restart waybar + mako + reload hyprland
...
# 7. Sync colors to global cache for SDDM access
