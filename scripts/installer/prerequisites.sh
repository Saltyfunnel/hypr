...
# Adjust config path: go up two levels from scripts/installer to repo root, then into config
CONFIGS_DIR="$(dirname "$(dirname "$SCRIPT_DIR")")/config"

echo "DEBUG: Configs directory resolved to: $CONFIGS_DIR"

# Append starship init if not present
...
# Append fastfetch command if not present
...
# --- Copy Catppuccin theme config files ---
if [ -d "$CONFIGS_DIR" ]; then
  echo "📁 Copying Catppuccin config files from $CONFIGS_DIR to $USER_HOME/.config/"
  ...
  ls -l "$CONFIGS_DIR/starship/starship.toml"

  sudo -u "$SUDO_USER" cp -rv "$CONFIGS_DIR/"* "$USER_HOME/.config/"
  ...
  echo "⚠️ Configs directory $CONFIGS_DIR not found. Skipping Catppuccin config copy."
fi
